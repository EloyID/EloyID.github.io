...
def create_batched_input_data_connection():
    ...
    try:
        ...
        if conn.is_connected():
            ...
            cursor.execute(
                """
                CREATE TABLE IF NOT EXISTS batched_input_data (
                    `Tipo Oferta` CHAR(1),
                    `Precio Compra/Venta` FLOAT,
                    `Fecha UTC` DATETIME,
                    `Energía Compra/Venta Acumulada` FLOAT,
                    UNIQUE KEY idx_datetime_type_energy (`Fecha UTC`, `Tipo Oferta`, `Energía Compra/Venta Acumulada`)
                )
                """
            )
    except Error as e:
